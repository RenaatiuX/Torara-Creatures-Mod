...
import rena.toraracreatures.util.TCHugeMushroom;

import java.util.Random;

public class TCMushroomPlantBlock extends BushBlock implements IGrowable {
    protected static final VoxelShape SHAPE = Block.box(5.0D, 0.0D, 5.0D, 11.0D, 6.0D, 11.0D);
    private final TCHugeMushroom mushroom;

    private final boolean isNetherFungi;

    public TCMushroomPlantBlock(Properties properties, TCHugeMushroom mushroom, boolean isNetherFungi) {
        super(properties);
    ...
    protected boolean mayPlaceOn(BlockState state, IBlockReader worldIn, BlockPos pos) {
        return (isNetherFungi) ? state.is(BlockTags.NYLIUM) || state.is(Blocks.MYCELIUM) || state.is(Blocks.SOUL_SOIL) || state.is(BlockTags.BASE_STONE_NETHER) || state.is(BlockTags.BASE_STONE_NETHER) || super.mayPlaceOn(state, worldIn, pos) : state.isSolidRender(worldIn, pos);
    }
