...
public class DeathCapMushroom3 extends TCAbstractMushroomFeature<TCMushroomConfig> {

    public DeathCapMushroom3(Codec<TCMushroomConfig> configCodec) {
        super(configCodec);
    }

    @Override
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, TCMushroomConfig config) {
        BlockState STEM_DIRT = config.getStemProvider().getState(rand, pos);
