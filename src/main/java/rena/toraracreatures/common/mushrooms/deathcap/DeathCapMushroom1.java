package rena.toraracreatures.common.mushrooms.deathcap;
...
public class DeathCapMushroom1 extends TCAbstractMushroomFeature<TCMushroomConfig> {

    public DeathCapMushroom1(Codec<TCMushroomConfig> configCodec) {
        super(configCodec);
    }

    @Override
    protected boolean placeMushroom(ISeedReader worldIn, Random rand, BlockPos pos, boolean isMushroom, TCMushroomConfig config) {
        BlockState STEM_DIRT = config.getStemDirtProvider().getState(rand, pos);
        ...
        if (pos.getY() + randTreeHeight + 1 < worldIn.getMaxBuildHeight()) {
            if (!isDesiredGroundwDirtTag(config, worldIn, pos.below(), Blocks.GRASS_BLOCK)) {
                ...
                placeStem(STEM_DIRT, worldIn, mainmutable.set(pos).move(0, 0, 0));
